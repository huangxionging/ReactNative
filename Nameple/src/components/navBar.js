...
export class NavBar extends React.Component {
  ...
  _renderLeft(headerLeft) {
    if (headerLeft) {
      return <View style={styles.left}>{headerLeft}</View>;
    }
    ...
    let renderLeftContent = () => {
      let index = _.findIndex(this.props.headerProps.scenes, {
        isActive: true
      });
      if (index > 0) {
        return (
    ...
    return <View style={[{ width }, styles.left]}>{renderLeftContent()}</View>;
  }
  ...
  render() {
    let options = this.props.headerProps.getScreenDetails(
      this.props.headerProps.scene
    ).options;
    return (
